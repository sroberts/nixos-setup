...
A NixOS flake configuration for a single host: a Framework 13 AMD (Ryzen 7040) laptop running niri + Noctalia (Quickshell) on an encrypted LVM-on-LUKS disk with hibernation-capable encrypted swap. It is the **source of truth** for the system — the running machine is a function of these files.
...
- **`flake.nix`** — Inputs (nixpkgs unstable, home-manager, nixos-hardware, niri-flake, noctalia, claude-code-nix) and the single `nixosConfigurations.sjr-fw13` output. All inputs `follows = "nixpkgs"` so there is one nixpkgs in the closure. The `lanzaboote` input is **commented out** by design — see *Secure Boot* below.
- **`configuration.nix`** — System-level: bootloader, LUKS resume target, PipeWire, networking, niri + greetd/tuigreet, Docker, Ollama (ROCm), the `sroberts` user, and the system-wide GUI apps (`_1password-gui`, `chromium`, `obsidian`, …). The function signature is `{ config, pkgs, inputs, … }`. When enabling Secure Boot, **`lib` must be added** to that signature.
- **`home.nix`** — User-level (home-manager): Noctalia config, niri input + binds, CLI/TUI tooling, zsh + integrations (zoxide, fzf, eza, bat, starship, mise), and `home.activation.*` hooks for the imperative gaps Nix can't declare (LazyVim starter, CyberChef download, post-install TODO.md).
