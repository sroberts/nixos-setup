...
The lock screen is Noctalia's own; it has its own PAM context. Lock/media/brightness keybinds in `home.nix` go through `loginctl lock-session`, `wpctl`, `playerctl`, and `brightnessctl` rather than shell-specific IPC, so they survive shell swaps.
